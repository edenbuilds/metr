...
/// - It reads local files written by the Codex and Claude Code CLIs. No network
///   calls, no credentials, no keychain access.
/// - It therefore reports **measured local activity**, not provider-reported
///   plan usage. No local file publishes your plan's limits, so the "used"
///   figure here is a proxy: this window's activity against your own busiest
///   comparable window. Every snapshot carries that caveat in
///   `sourceDescription`, and the UI shows it.
public final class LocalActivityDataSource: UsageDataSource {

    public let kind: DataSourceKind = .local

    public var provenance: String {
        "Reads ~/.codex/session_index.jsonl, file timestamps under ~/.claude/projects, and ~/.claude/stats-cache.json. No credentials, no network."
    }
