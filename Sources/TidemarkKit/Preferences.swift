...
public enum PresentationMode: String, CaseIterable, Codable, Sendable {
    ...
    public var title: String {
        switch self {
        case .top: return "Top island"
        case .side: return "Side edge"
        }
    }
...
public enum DataSourceKind: String, CaseIterable, Codable, Sendable {
    /// Reads real, credential-free files this machine already has.
    case local
    ...
    public var explanation: String {
        switch self {
        case .local: return "Reads agent session files already on this Mac. No credentials, no network."
...
public struct Preferences: Equatable, Codable, Sendable {
    ...
    // MARK: Location / timezone

    /// Master privacy switch for all location-aware behaviour.
    public var locationAware: Bool = true
    /// Nil means "follow the system".
    public var manualTimeZoneID: String?
    /// Optional label the user typed, e.g. "Mumbai".
    public var manualPlaceLabel: String?
    /// Show provider-timezone reset times alongside local ones when they differ.
    public var showProviderTimeZone: Bool = true
    ...
    public init(from decoder: Decoder) throws {
        ...
        locationAware = value(.locationAware, defaults.locationAware)
        manualTimeZoneID = try? c.decodeIfPresent(String.self, forKey: .manualTimeZoneID)
        manualPlaceLabel = try? c.decodeIfPresent(String.self, forKey: .manualPlaceLabel)
        showProviderTimeZone = value(.showProviderTimeZone, defaults.showProviderTimeZone)
    }

    public var timeZoneSource: TimeZoneSource {
        guard let manualTimeZoneID else { return .system }
        return .manual(identifier: manualTimeZoneID, label: manualPlaceLabel)
    }

    /// Wipe every stored location value. Used by the "Clear location data" control.
    public mutating func clearLocationData() {
        manualTimeZoneID = nil
        manualPlaceLabel = nil
    }
...
public final class PreferencesStore {
    ...
    public func load() -> Preferences {
        // `UP_PREFS` lets a test or a demo pin the app into an exact state
        // without touching the user's real preferences. Ignored when unset.
        if let json = ProcessInfo.processInfo.environment["UP_PREFS"],
    ...
    public var isOverriddenByEnvironment: Bool {
        ProcessInfo.processInfo.environment["UP_PREFS"] != nil
    }
    ...
    /// Remove every persisted preference, including location values.
