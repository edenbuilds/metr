...
public final class UsageStore: ObservableObject {
    ...
    @Published public var historyProviderID: String?
    ...
    public init(
        ...
        // Screenshot/release QA hooks. They change presentation only and never
        // alter provider data or persisted preferences.
        if let rawTab = ProcessInfo.processInfo.environment["METR_TAB"],
           let index = Int(rawTab), let tab = PanelTab(rawValue: index) {
            selectedTab = tab
        }
        historyProviderID = ProcessInfo.processInfo.environment["METR_HISTORY_PROVIDER"]
    }
    ...
    private static func makeDataSource(for kind: DataSourceKind, optionalAppIDs: Set<String> = []) -> UsageDataSource {
        ...
        case .mock:
            ...
            let name = ProcessInfo.processInfo.environment["METR_SCENARIO"] ?? ""
    ...
    // MARK: - System timezone

    /// macOS is the sole timezone authority. `autoupdatingCurrent` changes as
    /// the user travels without storing or requesting location information.
    ...
    public func refresh() async {
        ...
        snapshot = fresh
        emitDiagnostics(for: fresh)
        lastRefresh = captured
        now = captured
        evaluateAlerts(now: captured)
    }

    /// Opt-in, non-sensitive release diagnostic. This deliberately reports no
    /// usage values, session names, paths, or credentials.
    private func emitDiagnostics(for snapshot: UsageSnapshot) {
        guard ProcessInfo.processInfo.environment["METR_DIAGNOSTIC"] == "1" else { return }
        for provider in snapshot.providers {
            let line = "[metr] provider=\(provider.id) state=\(provider.state.label) confidence=\(provider.confidence.label) windows=\(provider.quotaPeriods.count) source=\(provider.sourceDescription)\n"
    ...
    public func resetDescription(for provider: ProviderSnapshot) -> ResetDescription {
        ...
        return ResetFormatter.describe(
            ...
            now: now,
            display: location,
            providerZone: location.timeZone
        )
    }
