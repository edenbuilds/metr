...
public final class MockUsageDataSource: UsageDataSource {
    ...
    private func providers(now: Date) -> [ProviderSnapshot] {
        ...
        func snapshot(
            ...
            ProviderSnapshot(
                ...
                quotaPeriods: fraction.map { value in
                    [
                        QuotaPeriod(label: "5-hour window", usedFraction: value, resetAt: now.addingTimeInterval(2.6 * 3600), span: 5 * 3600),
                        QuotaPeriod(label: "7-day window", usedFraction: min(1, value + 0.19), resetAt: now.addingTimeInterval(4.2 * 86_400), span: 7 * 86_400)
                    ]
                } ?? []
            )
        }
    ...
    private func sessions(now: Date) -> [SessionRecord] {
        ...
        return zip(titles, hourOffsets).enumerated().map { index, pair in
            SessionRecord(
                ...
                workingDirectory: "~/Projects/metr"
            )
        }
    }
    ...
    private func activity(now: Date) -> [DailyActivity] {
        ...
        return counts.enumerated().flatMap { index, count -> [DailyActivity] in
            guard let date = cal.date(byAdding: .day, value: -(counts.count - 1 - index), to: cal.startOfDay(for: now)) else { return [] }
            return [
                DailyActivity(
                    date: date,
                    messageCount: count,
