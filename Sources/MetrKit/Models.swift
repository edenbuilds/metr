...
/// One provider-reported limit window. Providers commonly expose both an
/// immediate window and a weekly window; retaining both keeps the slower limit
/// visible without obscuring the more urgent one.
public struct QuotaPeriod: Identifiable, Equatable, Sendable {
    public let label: String
    public let usedFraction: Double
    public let resetAt: Date?
    public let span: TimeInterval?

    public var id: String { "\(label)|\(Int(span ?? 0))" }

    public init(label: String, usedFraction: Double, resetAt: Date?, span: TimeInterval?) {
        self.label = label
...
public struct ProviderSnapshot: Identifiable, Equatable, Sendable {
    ...
    /// All provider-reported windows, shortest first. Empty for local estimates.
    public var quotaPeriods: [QuotaPeriod]
    ...
    public init(
        ...
        self.quotaPeriods = quotaPeriods
    }
...
public struct DailyActivity: Identifiable, Equatable, Sendable {
    ...
    /// The app that produced the activity, when its source identifies one.
    public let providerID: String?

    public var id: String { "\(providerID ?? "all")|\(date.timeIntervalSince1970)" }

    public init(date: Date, messageCount: Int, sessionCount: Int, toolCallCount: Int, providerID: String? = nil) {
        self.date = date
