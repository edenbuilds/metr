...
/// Renders the Tidemark mark as a menu-bar template image.
///
...
enum MenuBarIcon {
    ...
    static func image(level: Double, severity: Severity, isKnown: Bool) -> NSImage {
        ...
        image.accessibilityDescription = "Tidemark"
        cache[key] = image
        return image
    }

    // MARK: Geometry (mirrors TidemarkMark, in AppKit's bottom-left origin)
