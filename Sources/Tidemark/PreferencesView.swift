...
struct PreferencesView: View {
    ...
    var body: some View {
        TabView {
            ...
            locationTab.tabItem { Label("Location", systemImage: "globe") }
    ...
    private var panelTab: some View {
        Form {
            ...
            if store.preferences.mode == .side {
    ...
    private var alertsTab: some View {
        Form {
            ...
            Section("Quiet hours") {
                ...
                Text("Evaluated in \(store.location.inlinePhrase), so quiet hours move with you.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            VStack(alignment: .leading, spacing: Theme.Space.base) {
                ...
                GroupBox("What is real, and what is not") {
                    VStack(alignment: .leading, spacing: Theme.Space.snug) {
                        ...
                        bullet("Usage percentages are a local-activity proxy. No local file publishes your plan's limits, so this is not provider-reported usage.")
                        bullet("Costs are estimates from a stated assumption, shown next to the number. They are not billing data.")
                        bullet("There is no provider API integration and no network access of any kind.")
                    }
                    .padding(.vertical, 2)
                }
...
enum LoginItem {
    ...
    static var statusDescription: String {
        ...
        case .enabled: return "Registered. macOS will start Tidemark at login."
        case .requiresApproval: return "Approve Tidemark in System Settings › General › Login Items."
