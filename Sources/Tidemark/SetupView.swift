...
struct SetupCard: View {
        ...
        VStack(spacing: Theme.Space.snug) {
            ForEach(PresentationMode.allCases, id: \.self) { mode in
                ChoiceRow(
                    title: mode.title,
                    detail: mode == .top
                        ? "Sits under the menu bar, centred. Good on a big display."
                        : "Hugs the screen edge and collapses to a thin rail. Stays out of the way.",
    ...
    private var dataSourceStep: some View {
        VStack(spacing: Theme.Space.snug) {
            ...
            Text("No credentials are needed either way. Nothing is sent anywhere.")
                .font(.system(size: 9))
                .foregroundStyle(.tertiary)
    ...
    private var locationStep: some View {
        VStack(alignment: .leading, spacing: Theme.Space.snug) {
            LocationPicker(compact: true)
            Text("Taken from your Mac's timezone setting. No location permission is requested, and the app does not use GPS.")
                .font(.system(size: 9))
                .foregroundStyle(.tertiary)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private var alertsStep: some View {
        VStack(alignment: .leading, spacing: Theme.Space.snug) {
            ...
            if store.preferences.alertsEnabled {
                ...
                .toggleStyle(.switch)
                .controlSize(.small)
                Text("Quiet hours follow \(store.location.inlinePhrase), so they travel with you.")
                    .font(.system(size: 9))
                    .foregroundStyle(.tertiary)
            }
        }
