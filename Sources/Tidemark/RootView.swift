        ...
        HStack(spacing: Theme.Space.base) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(spacing: Theme.Space.tight) {
                    ...
                    if store.preferences.locationAware && store.location.isOverridden {
                ...
                .accessibilityLabel("Showing \(store.location.inlinePhrase). Updated \(store.lastRefresh.map { Formatters.relativeUpdated($0, now: store.now) } ?? "never").")
            }
            ...
            DockRow {
                ...
                DockItem {
                    ChipButton(symbolName: "xmark", help: "Hide Tidemark — reopen from the menu bar (⌘W)") {
                        actions.close()
                    }
                }
            }
...
/// it is recognisably Tidemark rather than a stray line on the screen edge.
struct RailView: View {
    ...
    private var isVertical: Bool { store.preferences.mode == .side }
