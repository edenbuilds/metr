...
final class DataSourceTests: XCTestCase {
    ...
    private func makeFixtureHome() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("metr-tests-\(UUID().uuidString)")
    ...
    func testLocalAdapterReadsSessionsAndActivityFromDisk() async throws {
        ...
        let claudeActivity = snapshot.activity.filter { $0.providerID == KnownProvider.claude.id }
        let codexActivity = snapshot.activity.filter { $0.providerID == KnownProvider.codex.id }
        XCTAssertEqual(claudeActivity.count, 2)
        XCTAssertEqual(claudeActivity.first?.messageCount, 100)
        XCTAssertEqual(claudeActivity.last?.messageCount, 250, "Claude activity must come back in date order")
        XCTAssertEqual(codexActivity.reduce(0) { $0 + $1.sessionCount }, 3)
        XCTAssertTrue(codexActivity.allSatisfy { $0.messageCount == 0 }, "Codex does not expose message totals here")
    }
    ...
    func testLocalAdapterReportsUnavailableRatherThanInventingNumbers() async throws {
        ...
        let home = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("metr-empty-\(UUID().uuidString)")
