...
final class StateAndPreferencesTests: XCTestCase {
    ...
    func testPreferencesRoundTripThroughDefaults() {
        ...
        prefs.mode = .top
        prefs.manualTimeZoneID = "Europe/London"
        prefs.manualPlaceLabel = "Home"
        ...
        XCTAssertEqual(loaded.mode, .top)
        XCTAssertEqual(loaded.manualTimeZoneID, "Europe/London")
        XCTAssertEqual(loaded.manualPlaceLabel, "Home")
    ...
    func testClearingLocationDataRemovesBothTheZoneAndTheLabel() {
        var prefs = Preferences()
        prefs.manualTimeZoneID = "Asia/Kolkata"
        prefs.manualPlaceLabel = "Mumbai"
        prefs.clearLocationData()
        XCTAssertNil(prefs.manualTimeZoneID)
        XCTAssertNil(prefs.manualPlaceLabel)
        XCTAssertEqual(prefs.timeZoneSource, .system)
    }
...
final class ForwardCompatibilityTests: XCTestCase {
    ...
    @MainActor
    func testQuietHoursFollowAManualTimezoneOverride() async {
        let store = UsageStore(
            preferencesStore: PreferencesStore(defaults: Fixture.defaults()),
            onboardingStore: OnboardingStore(defaults: Fixture.defaults()),
            dataSource: MockUsageDataSource(scenario: .healthy)
        )
        store.preferences.quietHours = QuietHours(enabled: true, startHour: 22, endHour: 8)

        // Pick the zone where it is currently 23:00, and one where it is 12:00.
        let night = zoneWhereLocalHour(is: 23)
        let day = zoneWhereLocalHour(is: 12)

        store.setManualTimeZone(night.identifier, label: nil)
        XCTAssertTrue(store.isInQuietHours, "Quiet hours must move with the chosen location")

        store.setManualTimeZone(day.identifier, label: nil)
        XCTAssertFalse(store.isInQuietHours)
    }

    /// Find a real timezone in which the current instant is a given local hour.
    private func zoneWhereLocalHour(is hour: Int) -> TimeZone {
        let now = Date()
        for identifier in TimeZone.knownTimeZoneIdentifiers {
            guard let zone = TimeZone(identifier: identifier) else { continue }
            var cal = Calendar(identifier: .gregorian)
            cal.timeZone = zone
            if cal.component(.hour, from: now) == hour { return zone }
        }
        return .current
    }
}
